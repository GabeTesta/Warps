...
{
	//[Serializable()]
	class Geodesic: MouldCurve
	{
		public Geodesic():this("", null, null ){}
		public Geodesic(string label, Sail sail, IFitPoint[] fits)
			:base(label, sail)
		{
			if (fits != null)
				Fit(fits);
		...
		//			return null;
		//		Vect3[] x = new Vect3[m_uvs.Length];
		//		for (int i = 0; i < m_uvs.Length; i++)
		//		{
		//			x[i] = new Vect3();
		//			xVal(m_uvs[i], ref x[i]);
		//		}
		//		return x;
		//	}
		//}

		public bool Geo(IFitPoint start, IFitPoint end)
		{ return Geo(this, start, end); }
		public static bool Geo(MouldCurve g, IFitPoint start, IFitPoint end)
		{
			...
			////interpolate 8 internal points in uv
			//Vect2 uI = new Vect2();
			//double s;
			//for (i = 1; i < INC; i++)
			//{
			//	s = (double)i / (double)INC;
			//	for (int j = 0; j < 2; j++)
			//		uI[j] = BLAS.interpolate(s, uEnd[j], uStart[j]);
			//	SetFitPnt(i, uI);
			//	SetFitPos(i, s);
			//}
		...
		static void RefineGeo(MouldCurve g, ref double[] sFits, ref Vect2[] uFits)
		{
			...
			for (int i = 1; i < NumFits; i++)
			{
				...
				//GetFitPnt(uFits, i - 1, ref um);
				//GetFitPnt(uFits, i, ref up);
				//dmu = um - uv;// BLAS.subtract(um, uv);
				//dpu = up - uv;// BLAS.subtract(up, uv);
				dmu = uFits[i - 1] - uv;
				dpu = uFits[i] - uv;

				//dml = dmu.Magnitude;// BLAS.magnitude(dmu);
				//dpl = dpu.Magnitude;// BLAS.magnitude(dpu);
				...
				dmx = dmx - xyz;
				dpx = dpx - xyz;
				//dmx = BLAS.subtract(dmx, xyz);
				//dpx = BLAS.subtract(dpx, xyz);

				//dml = BLAS.magnitude(dmx);
				//dpl = BLAS.magnitude(dpx);
